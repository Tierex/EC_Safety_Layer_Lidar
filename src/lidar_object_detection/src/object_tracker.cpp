...
struct Track
{
  ...
  int misses = 0;
  int age = 0;
  double last_source_time_sec = 0.0;
...
public:
  ...
  : Node("object_tracker")
  {
    ...
    auto qos = rclcpp::QoS(rclcpp::KeepLast(qos_depth_)).reliable().durability_volatile();

    sub_ = this->create_subscription<std_msgs::msg::Float32MultiArray>(
      input_topic_, qos, std::bind(&ObjectTracker::callback, this, _1));
...
private:
  ...
  void loadParameters()
  {
    ...
    if (max_missed_frames_ < 0) max_missed_frames_ = 0;
    if (min_hits_to_publish_ < 1) min_hits_to_publish_ = 1;
    if (debug_every_n_frames_ < 1) debug_every_n_frames_ = 1;
    velocity_alpha_ = std::clamp(velocity_alpha_, 0.0, 1.0);
    if (min_dt_ <= 0.0) min_dt_ = 0.001;
    if (max_dt_ < min_dt_) max_dt_ = min_dt_;
  }

  std::vector<Detection> parseDetections(const std_msgs::msg::Float32MultiArray & msg)
  {
    ...
    if (stride != 11 && stride != 8) {
      if (msg.data.size() % 11 == 0) stride = 11;
      else if (msg.data.size() % 8 == 0) stride = 8;
      else return dets;
    }
    ...
    for (std::size_t i = 0; i < n; ++i) {
      ...
      if (stride >= 11) {
        ...
        d.detector_frame = msg.data[k + 10];
      }
      dets.push_back(d);
    }
  ...
  static double xyDistance2Predicted(const Track & t, const Detection & d, double dt)
  {
    const double px = static_cast<double>(t.x) + static_cast<double>(t.vx) * dt;
    const double py = static_cast<double>(t.y) + static_cast<double>(t.vy) * dt;
    const double dx = px - static_cast<double>(d.x);
    const double dy = py - static_cast<double>(d.y);
    return dx * dx + dy * dy;
  }

  bool passesGates(const Track & t, const Detection & d, double dt) const
  {
    const double xy2 = xyDistance2Predicted(t, d, dt);
    if (xy2 > max_match_distance_ * max_match_distance_) return false;

    const double z_diff = std::abs(static_cast<double>(t.z) - static_cast<double>(d.z));
    if (z_diff > max_z_match_distance_) return false;

    if (max_size_change_ratio_ > 1.0) {
      if (!sizeCompatible(t.dx, d.dx) || !sizeCompatible(t.dy, d.dy) || !sizeCompatible(t.dz, d.dz)) {
        return false;
      }
    }
  ...
  void callback(const std_msgs::msg::Float32MultiArray::SharedPtr msg)
  {
    ...
    const rclcpp::Time wall_now = this->now();
    double wall_dt = (wall_now - last_wall_time_).seconds();
    ...
    double dt = wall_dt;
    if (source_time > 0.0 && last_source_time_sec_ > 0.0) {
      const double source_dt = source_time - last_source_time_sec_;
      if (source_dt > 0.0) dt = source_dt;
    }
    if (source_time > 0.0) {
      last_source_time_sec_ = source_time;
    }
    ...
    for (std::size_t ti = 0; ti < tracks_.size(); ++ti) {
      for (std::size_t di = 0; di < dets.size(); ++di) {
        if (!passesGates(tracks_[ti], dets[di], dt)) continue;
        candidates.push_back(MatchCandidate{ti, di, xyDistance2Predicted(tracks_[ti], dets[di], dt)});
      }
    }

    std::sort(
      candidates.begin(), candidates.end(),
      [](const MatchCandidate & a, const MatchCandidate & b) { return a.cost < b.cost; });

    std::vector<bool> track_used(tracks_.size(), false);
    std::vector<bool> det_used(dets.size(), false);

    for (const auto & c : candidates) {
      if (track_used[c.track_idx] || det_used[c.det_idx]) continue;
    ...
    tracks_.erase(
      std::remove_if(
        ...
        [&](const Track & t) { return t.misses > max_missed_frames_; }),
      tracks_.end());

    const double tracker_ms = elapsedMs(t0);
    publishTracks(source_time, tracker_ms, detector_ms);
    if (publish_markers_) publishMarkers(wall_now);
    ...
    if (debug_ && (frame_count_ % debug_every_n_frames_) == 0) {
      RCLCPP_INFO(
        ...
        frame_count_, dets.size(), tracks_.size(), dt, detector_ms, tracker_ms);
    }
  }

  void updateMatchedTrack(Track & t, const Detection & d, double dt)
  {
    const float vx_meas = clampVelocity(static_cast<float>((d.x - t.x) / dt));
    const float vy_meas = clampVelocity(static_cast<float>((d.y - t.y) / dt));
    const float vz_meas = clampVelocity(static_cast<float>((d.z - t.z) / dt));

    t.vx = static_cast<float>((1.0 - velocity_alpha_) * t.vx + velocity_alpha_ * vx_meas);
    t.vy = static_cast<float>((1.0 - velocity_alpha_) * t.vy + velocity_alpha_ * vy_meas);
    t.vz = static_cast<float>((1.0 - velocity_alpha_) * t.vz + velocity_alpha_ * vz_meas);
    ...
    ++t.hits;
    t.misses = 0;
    t.last_source_time_sec = d.source_time_sec;
  ...
  std::size_t countPublishableTracks() const
  {
    ...
    for (const auto & t : tracks_) {
      if (t.hits >= min_hits_to_publish_) ++count;
    }
    return count;
  }

  void publishTracks(double source_time_sec, double tracker_processing_ms, double detector_processing_ms)
  {
    ...
    out.layout.dim[0].stride = static_cast<uint32_t>(publish_count * fields_per_track_);
    ...
    for (const auto & t : tracks_) {
      ...
      const double stamp = t.last_source_time_sec > 0.0 ? t.last_source_time_sec : source_time_sec;
      const double det_ms = t.last_detector_processing_ms > 0.0 ? t.last_detector_processing_ms : detector_processing_ms;
  ...
  void setLifetime(visualization_msgs::msg::Marker & m) const
  {
    ...
    const int nsec = static_cast<int>((marker_lifetime_ - static_cast<double>(sec)) * 1e9);
    m.lifetime.sec = sec;
